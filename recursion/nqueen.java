...
public class nqueen {
    static void putqueen(int col,char[][] board,List<List<String>> list,int[] leftrow,int[] upperdiagonal,int[] lowerdiagonal){
        if(col==board.length){
            list.add(construct(board));
            return;
        }
        for(int row=0;row<board.length;row++){
            if(leftrow[row]==0 && upperdiagonal[row+col]==0 && lowerdiagonal[board.length-1+row-col]==0){
                board[row][col]='Q';
                leftrow[row]=1;
